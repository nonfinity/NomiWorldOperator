...
// prep the inspector
let sidetray = new inspector(`inspector`);
    sidetray.clear();
    sidetray.load(world.getHubByName('Dawnstar'));


// how to register a forcemap click event from here?
let gHubs = document.querySelectorAll(`g.hubs > circle`)
for( let i of gHubs) {
  i.addEventListener('click', (e:Event) => { hub_click(+i.innerHTML)})
  i.addEventListener('touchend', (e:Event) => { hub_click(+i.innerHTML)})
}

gHubs = document.querySelectorAll(`g.links > line`)
for( let i of gHubs) {
  i.addEventListener('click', (e:Event) => { edge_click(+i.innerHTML)})
  i.addEventListener('touchend', (e:Event) => { edge_click(+i.innerHTML)})
}
 ...
 */ 
  ...
  function hub_click(hub_id: number) {
    // console.log(`clicked on Hub with ID = ${hub_id}`)
    sidetray.load( world.getHubByID(hub_id) );
  }

  function edge_click(edge_id: number) {
    // console.log(`clicked on Edge with ID = ${edge_id}`)
    sidetray.load( world.getEdgeByID(edge_id) );
  }
// * --------------------------------------